...
void jaime3() {
	...
	do {
		...
		case 2: cout << "Ingrese la Lista que desee imprimir: ";
			...
			imprimir(valor);
			break;
		case 3: cout << "Ingrese el valor a buscar: ";
			cin >> valor;
			for (int i = 0; i < 14; i++) {
...
/*Douglas1 Floy Warshall*/
void BuscarCamino() {
	system("cls");
	int i, j;
	cout << "Vertice de salida: " << endl;
	cin >> i;
	cout << "Vertice destino: " << endl;
	cin >> j;
	if (i == 0 || j == 0 || i == j)
		cout << "La distancia minima es 0";
	else {
		cout << "La distancia minima entre los nodos es de: " << endl;
		cout << Matrizad[i][j] << endl;
		cout << "pasa por el " << Matrizr[i][j] << " y despues por el " << j << endl;
	}
	_getch();
}
void mostrarmaxuno() {
...
void Calcular() {
	...
	int bucle, i, j, suma;
	for (bucle = 1; bucle <= nodos; bucle++) {
		for (i = 1; i <= nodos; i++) {
			vectorA[i] = Matrizad[bucle][i];
			vectorB[i] = Matrizad[i][bucle];
		}
		for (i = 1; i <= nodos; i++)
			for (j = 1; j <= nodos; j++) {
				if (vectorB[i] == 999 || vectorA[j] == 999) suma = 999;
				else suma = vectorB[i] + vectorA[j];
				if (suma<Matrizad[i][j]) {
					Matrizad[i][j] = suma;
					Matrizr[i][j] = bucle;
				}
			}
...
void Douglas1()
{
	...
	{
		system("cls");
		gotoxy(25, 6);  cout << "===== MENU PRINCIPAL =====" << endl;
		gotoxy(20, 10); cout << "1. Agregar datos a matriz" << endl;
		gotoxy(20, 11); cout << "2. Mostrar matriz" << endl;
		gotoxy(20, 12); cout << "3. Mostrar matriz de estados" << endl;
		gotoxy(20, 13); cout << "4. Encontrar camino" << endl;
		gotoxy(20, 14); cout << "5. Salir" << endl;
		gotoxy(20, 17); cout << "Ingrese opcion: ";
		gotoxy(35, 17); cin >> opcion;
		...
		case 1:
			...
			if (nodos != tam)
			{
				cout << "Tiene que ser el mismo tamanho de columnas y filas" << endl;
				system("pause");
				exit(0);
			}
			...
			for (i = 1; i <= nodos; i++)
				for (j = 1; j <= nodos; j++) {
					if (i != j) {
						cout << "ingrese valor para la posicion: " << i << "x" << j << ": ";
						cin >> Matrizad[i][j];
...
/*Douglas2 Liatasdoblemente enlzadas*/
	...
	void ingresoEspecifico()
	{

		if (inicio == NULL)
		{
			cout << "no se encuentra ningun dato" << endl;
		}
		else if (inicio->siguiente != NULL)
		{

			inicio->num = valores[10];


		}
		else if (inicio->num = valores[10])
		{
			cout << "ingrese la posicion donde desea ingresar el valor" << endl;
			for (int i = 0; i <= valores[10]; i++)
			{
				cin >> i;
				valores[i] = inicio->num;
				cout << "valor agregado";
			}
		}
	}
	...
	void Douglas2() 
	{
...

		{
			...
			cout << "5. Ingresar en posicion especifica" << endl;
			cout << "6. Salir" << endl;
...
int main() {
	...
	do {
		...
		gotoxy(25, 14); cout << "       6. Floyd Warshall								 " << endl;
		gotoxy(25, 15); cout << "       7. Listas Doblemente Enlazadas					 " << endl;
		gotoxy(25, 16); cout << "       8. cosa											 " << endl;
		gotoxy(25, 17); cout << "       9. SALIR                                           " << endl;
